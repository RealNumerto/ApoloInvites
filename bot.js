...
client.on("guildMemberAdd", (member) => {
    ...
    guild.fetchInvites().then(invites => {
        ...
        if(channel){
            ...
            .replace("-target-", `${invite.inviter.username + "#" + invite.inviter.discriminator}`)
...
client.on("guildMemberRemove", (member) => {
    ...
    var total = 0, bonus = 0, regular = 0, fakecount = 0, channel = member.guild.channels.cache.get(settings.Channel), content = settings.leaveMessage ? settings.leaveMessage : `-member- je izašao/la servera.`, data = db.get(`invites.${member.id}`);
    if(!data){
        if(channel){
            content = content
            .replace("-member-", `${member}`);
    ...
    if(channel){
        content = content
        .replace("-member-", `${member}`)
        .replace("-target-", `${im ? im.user.tag : data.inviter.user.tag}`) //////${im ? im : data.inviter}/////
